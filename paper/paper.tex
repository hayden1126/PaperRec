...
ranking we want comes out as the stationary distribution of a
...
is the \emph{stationary distribution} of a \emph{Markov chain},
...
stochastic transition matrix, the stationary distribution
...
$P$ \emph{column-stochastic} when every column sums to $1$, that
...
before $P$ is properly column-stochastic.
...
column-stochastic, and it preserves the random-walk
interpretation of the eventual stationary
distribution~\cite{gleich2015pagerank}.

With $P$ now stochastic, we define the teleport-modified transition
matrix
\begin{equation}
...
stochastic vectors, $M$ is column-stochastic and therefore a
...
P_{jj})$ by column-stochasticity. Whenever $d < 1$, the diagonal
...
(column-stochasticity, again) gives $(1-d)\,\mathbf{1}^\top
\mathbf{v} = (1-d)$, hence $\mathbf{1}^\top \mathbf{v} = 1$.

\section{Method A: Power Iteration as a Recursive Sequence}
\label{sec:method-a}

Equation~\eqref{eq:ppr-linear} hands us $\mathbf{v}$ in one
shot, but a single solve tells us nothing about how a walker
actually settles into the stationary distribution. Power
iteration recovers that picture by trading the solve for a
step-by-step update rule, and the rate at which that update
converges is what we set out to predict in this section:
\begin{equation}
...
Nicholson~\cite{nicholson2018linear}, and unrolling it exposes
the structure the linear solver hides.
...
behaves as $k$ grows. First, $P$ is column-stochastic, so
$\mathbf{1}^\top P = \mathbf{1}^\top$. The all-ones row vector is
therefore a left eigenvector of $P$ with eigenvalue $1$, so $1$
is an eigenvalue of $P$. Second, every eigenvalue of a column-stochastic
...
= 1$ is a stationary distribution of $P$, with nonnegative
...
simultaneously. As a
Markov chain parameter, $d$ is the probability that the walker
continues along the graph rather than teleporting back to the
seed. As a linear-algebra parameter, $d$ is the contraction
factor governing how fast~\eqref{eq:power-iter} forgets its
starting condition. The two roles are not independent: every
teleport resets the walker to the known state $\mathbf{e}_s$,
which shrinks the non-stationary components of the current
iterate by a factor of $d$. A smaller $d$ teleports more often,
converges faster, and biases scores toward the seed; a larger
$d$ explores more of the graph but converges more slowly.
